...
#include "ros_echronos/include/ros.hpp"
#include "ros_echronos/include/Message.hpp"
...
class pwm_ : Message {
  ...
  virtual inline void generate_block() {
      size_t offset = 0;
      memcpy(buffer+offset, &joint, sizeof(joint));
      offset+=sizeof(joint);
      memcpy(buffer+offset, &pwm, sizeof(pwm));
      offset+=sizeof(pwm);
      memcpy(buffer+offset, &targetVel, sizeof(targetVel));
      offset+=sizeof(targetVel);
      memcpy(buffer+offset, &currentVel, sizeof(currentVel));
      offset+=sizeof(currentVel);
      memcpy(buffer+offset, &currentPos, sizeof(currentPos));
      offset+=sizeof(currentPos);
      memcpy(buffer+offset, &targetPos, sizeof(targetPos));

...
//    owr_messages::board msg;
    ...
    owr_messages::board out_msg;
    strncpy(out_msg.joint, "aaaa", 4);
    out_msg.board = 0xDEADBEEF;

    // create a publisher
    Publisher<owr_messages::board> pub("null", board_buffer, 5, false);
    ...
    // create a subscriber
    owr_messages::board board_buffer_in[5];
    Subscriber<owr_messages::board> sub("null", board_buffer_in, 5, callback);


    // read them back
    owr_messages::board in_msg;
    for(int i = 0; i < 7; ++i) {
...
void callback(const owr_messages::board & outer_msg) {
    const owr_messages::pwm msg = outer_msg.joints[0];
